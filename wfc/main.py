...
def main():
    run_render(input_matrix, seed=None)
...
def gen_model(matrix, seed=None, height=10, width=50):
    if seed is not None:
        random.seed(seed)
    compatibilities, weights = parse_example_matrix(matrix)
    # compatibilities2, weights2 = parse_example_matrix(input_matrix2)
    compatibility_oracle = CompatibilityOracle(compatibilities)
    model = Model((height, width), weights, compatibility_oracle)
    return model


def gen_run(*a, **kw):
    model = gen_model(*a, **kw)
    return model.run()
