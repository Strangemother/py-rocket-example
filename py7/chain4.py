...
class Unit(object):
    ...
    def minor(self):
        ...
        self.charge_slice = Goumbles(v, volts=self._power.volts, amps=self._power.amps)
        # v = self.charge * cv * (self.resistance)
        self.charge -= v
        print('  ',self.name, f'{self.charge:.2f}, sending {v:.2f}')
        self.charge_slice.count = v
        self.table.send_to_next(self, self.charge_slice)
...
class Battery(Unit):
    ...
    def setup(self):
        power = Power(volts=self.volts, amps=1)
        self._power = power
        self.charge = power.goubmles()
    ...
    def major(self, table, units):
        # Emit a power event.
        #print(self.name, 'major power tick', self._power)

        self.charge = self._power.goubmles()
        # table.send_to_next(self, self._power)
